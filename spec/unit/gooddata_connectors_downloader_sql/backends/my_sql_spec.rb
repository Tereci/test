...
describe GoodData::Connectors::DownloaderSql::Backend::MySql do
  ...
  let(:obj) do
    obj = GoodData::Connectors::DownloaderSql::Backend::MySql.new(metadata)
    obj.connection_options = {
      'server' => 'svs-82-mysql.cmklrh2wh3es.us-east-1.rds.amazonaws.com:3306',
      'database' => 'gd_test',
      'username' => 'gooddata',
      'password' => ENV['GD_TEST_MYSQL_PASS']
    }
    obj
  end

  it 'Is defined' do
    expect(GoodData::Connectors::DownloaderSql::Backend::MySql)
    expect(GoodData::Connectors::DownloaderSql::Backend::MySql).to be_kind_of(Class)
  end
  ...
  describe '#create_connection' do
    ...
    it 'Creates connection with fetch size' do
      ...
      obj.create_connection
      expect(obj.db).to be_instance_of(Sequel::JDBC::Database)
      expect(obj.db.fetch_size).to eq 50
    end
  end
  ...
  describe '#load_db_fields' do
    it 'Loads fields' do
      ...
      expect(fields.map { |field| field.type.type }).to eq %w(integer string decimal date boolean string)
    end
  end

      ...
      module Backend
        class MsSql < BaseBackend
          ...
          def create_connection
            @db = Sequel.connect("jdbc:sqlserver://#{@connection_options['server']};database=#{@connection_options['database']}", user: @connection_options['username'], password: @connection_options['password'], selectMethod: 'cursor', packetSize: 0)
          ...
          def load_db_fields(metadata_entity, schema = nil)
            ...
            columns_response.each do |column|
              ...
              when 35
                type = 'string-255'
              when 47, 99, 37, 39, 45
                type = "string-#{column[:length] <= 0 ? 255 : column[:length]}"
                # Date
                # Timestamp
              when 61, 58, 111
                type = 'date-true'
